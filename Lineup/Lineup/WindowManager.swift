...
class WindowManager: ObservableObject {
    ...
    private func dismissSwitcherWindow(rideTransition: Bool) {
        ...
        let finish = { [weak self] in
            // A new switcher may have opened during the fade; don't tear down its
            // window/content in that case.
            guard let self = self, !self.isShowingSwitcher, !self.isShowingAppSwitcher else { return }
            w.orderOut(nil)
            w.alphaValue = 1.0
    ...
    private func spaceIndexMap() -> [CGSSpaceID: Int] {
        ...
        for display in displays {
            if let spaces = display["Spaces"] as? [NSDictionary] {
                for space in spaces {
                    // Only number type-0 (regular) desktops so the badge matches
                    // Mission Control; full-screen Spaces (type != 0) are skipped.
                    let isUserSpace = (space["type"] as? Int ?? 0) == 0
                    if isUserSpace, let id = space["id64"] as? CGSSpaceID {
                        map[id] = index
                        index += 1
                    }
                }
